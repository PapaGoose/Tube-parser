...
regex_dict = {
    ...
    # "Размер":r'(\d+\S{0,1}\d+\s*(?:x|х)\s*\d{1,2}\S{0,1}\d{1,2})',
    "Размер":r'(\d{2,4}\,\d{1,2}(?:x|х)\d{1,2}\,\d{1,2}|\d{2,4}(?:x|х)\d{1,2})',
    ...
    "Тип резьбы" : r'[Г-РТ]{4}|[В-М]{3}-\d|[A-Z]{3}\s[A-Z]{5}|[Б][ТС]{2}|[Н][КТ]{2}\s[Г]{0,1}|[В][С]|[NU]{2}',
    ...
    "Класс прочности" : r'(?<=прочности )\w?\d{1,3}|не ниже \w?\d{1,3}|не ниже K\d{1,3}|Не ниже \d{1,3}|не ниже \d{1,3}',
